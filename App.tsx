...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen noir-gradient flex flex-col selection:bg-white selection:text-black pb-20 lg:pb-0">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="glass-card p-5 lg:p-6 rounded-3xl space-y-6 lg:sticky lg:top-24">
            <div>
              ...
              {!originalImage ? (
                  ...
                  {isCameraOpen ? (
                    <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-black border border-neutral-800">
                      ...
                      <div className="absolute bottom-6 left-0 right-0 flex justify-center items-center gap-6">
                        <button onClick={flipCamera} className="p-3 bg-white/10 backdrop-blur-md rounded-full text-white"><FlipHorizontal className="w-5 h-5" /></button>
                        <button onClick={capturePhoto} className="w-16 h-16 bg-white rounded-full flex items-center justify-center border-4 border-neutral-400"><Aperture className="w-8 h-8 text-black" /></button>
                        <button onClick={stopCamera} className="p-3 bg-red-500/80 backdrop-blur-md rounded-full text-white"><X className="w-5 h-5" /></button>
                      </div>
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 gap-3">
                      <div onClick={startCamera} className="aspect-[3/4] rounded-2xl border border-neutral-800 hover:border-neutral-600 flex flex-col items-center justify-center cursor-pointer bg-neutral-900/30">
                        <Camera className="w-6 h-6 text-neutral-400" /><p className="mt-2 text-xs text-neutral-400 font-medium">Take Photo</p>
                      </div>
                      <div onClick={() => fileInputRef.current?.click()} className="aspect-[3/4] rounded-2xl border border-neutral-800 hover:border-neutral-600 flex flex-col items-center justify-center cursor-pointer bg-neutral-900/30">
                ...
                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden group">
                  ...
                  <button onClick={clearWorkspace} className="absolute top-2 right-2 p-2 bg-black/60 rounded-full text-white"><X className="w-4 h-4" /></button>
                </div>
              )}
            ...
            <div>
              ...
              <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={5} className="w-full bg-neutral-900/50 border border-neutral-800 rounded-xl p-4 text-sm text-neutral-300 focus:outline-none focus:ring-1 focus:ring-white/20 resize-none leading-relaxed" placeholder="Describe the cinematic style..." />
            </div>

            <Button className="w-full py-4 text-base" size="lg" onClick={handleGenerate} isLoading={status === AppStatus.GENERATING} disabled={!originalImage}>
              <Wand2 className="w-5 h-5 mr-2" /> Develop Portrait
            </Button>
        ...
        <div id="output-canvas" className="lg:col-span-8">
          <div className="glass-card rounded-3xl min-h-[500px] flex flex-col p-5 lg:p-8 bg-neutral-900/10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              ...
              {generatedImage && (
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" onClick={handleDownload} className="flex-1 sm:flex-initial"><Download className="w-4 h-4 sm:mr-2" /><span className="hidden sm:inline">Download</span></Button>
                  <Button variant="secondary" size="sm" onClick={handleGenerate} className="flex-1 sm:flex-initial"><RefreshCcw className="w-4 h-4 sm:mr-2" /><span className="hidden sm:inline">Refine</span></Button>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 flex items-center justify-center bg-black/40 rounded-2xl relative overflow-hidden border border-white/5 min-h-[400px]">
              {status === AppStatus.GENERATING ? (
                ...
                <div className="relative group w-full h-full flex items-center justify-center p-4">
                   <img src={generatedImage} className="max-h-[70vh] w-auto shadow-2xl rounded ring-1 ring-white/10" alt="Generated Masterpiece" />
      ...
      {history.length > 0 && (
        <section className="bg-black py-16 lg:py-24 border-t border-neutral-900">
          <div className="max-w-7xl mx-auto px-4 lg:px-6">
            <h2 className="text-2xl lg:text-3xl italic font-serif mb-8 lg:mb-12">Studio Archives</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-8">
              {history.map((item) => (
                <div key={item.id} className="group glass-card rounded-2xl overflow-hidden hover:scale-[1.02] transition-all">
                  <div className="aspect-[3/4] relative">
                    <img src={item.generatedImage} className="w-full h-full object-cover" alt="History" />
                    <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-end gap-2">
                       <Button variant="primary" size="sm" className="w-full text-[10px]" onClick={() => { setGeneratedImage(item.generatedImage); setOriginalImage(item.originalImage); setPrompt(item.prompt); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>Restore</Button>
                       <Button variant="outline" size="sm" className="w-full text-[10px]" onClick={() => { const link = document.createElement('a'); link.href = item.generatedImage; link.download = `archive-${item.id}.png`; link.click(); }}>Download</Button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <footer className="py-8 lg:py-12 border-t border-neutral-900 text-center">
        <div className="flex items-center justify-center gap-2 mb-4">
          <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
          <span className="text-[10px] uppercase tracking-widest text-neutral-600 font-bold">Studio Active</span>
        </div>
        <p className="text-neutral-500 text-[10px] lg:text-xs">Noir Portrait Studio • Cinematic AI Transformation</p>
      </footer>
    </div>
  );
};
